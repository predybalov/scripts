...
case "$registry" in
    ...
    "<two>"    ) cat ${keydir}/${registry}.json | docker login --username json_key --password-stdin cr.yandex && \
                      echo -e "to ${GREEN}${BOLD}${registry}${NC} registry";;
    "<three>"  ) cat ${keydir}/${registry}.json | docker login --username json_key --password-stdin cr.yandex && \
                      echo -e "to ${GREEN}${BOLD}${registry}${NC} registry";;
